...
const INACTIVE_DAYS = 30; // One month of inactivity

/**
 * Calculate exact threshold: exactly 30 days ago.
 */
...
/**
 * Calculate threshold for start of 30 days ago (midnight).
 */
...
/**
 * Find all inactive chats (no message for 30+ days)
 *
 * A chat is inactive if:
 * 1. lastMessageAt is null AND startedAt is 30+ days ago, OR
 * 2. lastMessageAt exists AND is 30+ days ago
 *
 * NOT inactive if:
 * - lastMessageAt is within last 30 days
...
export const findInactiveChats = async () => {
  try {
    ...
    console.log(`[ChatCleanup] Threshold date (${INACTIVE_DAYS} days ago): ${thresholdDate.toISOString()}`);
    ...
    console.log(`[ChatCleanup] Found ${inactiveChats.length} chats older than ${INACTIVE_DAYS} days`);
